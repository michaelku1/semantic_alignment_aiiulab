...
def main(cfg):
    ...
    dataset_val_cross = build_dataset(image_set='val_target_like_source', cfg=cfg)
    ...
    if cfg.DIST.DISTRIBUTED:
        if cfg.CACHE_MODE:
            ...
            sampler_val_tgt_like_src = samplers.NodeDistributedSampler(dataset_val_cross, shuffle=False)
        else:
            sampler_train = samplers.DistributedSampler(dataset_train)
            sampler_val_src = samplers.NodeDistributedSampler(dataset_val_src, shuffle=False)
            sampler_val_tgt = samplers.DistributedSampler(dataset_val_tgt, shuffle=False)
            sampler_val_tgt_like_src = samplers.DistributedSampler(dataset_val_cross, shuffle=False)
    else:
        ...
        sampler_val_tgt_like_src = torch.utils.data.SequentialSampler(dataset_val_cross, shuffle=False)
    ...
    data_loader_val_tgt = DataLoader(dataset_val_tgt, cfg.TRAIN.BATCH_SIZE, sampler=sampler_val_tgt,
                                     drop_last=False, collate_fn=utils.collate_fn, num_workers=cfg.NUM_WORKERS,
                                     pin_memory=True)
    data_loader_val_cross = DataLoader(dataset_val_cross, cfg.TRAIN.BATCH_SIZE, sampler=sampler_val_tgt_like_src,
                                       drop_last=False, collate_fn=utils.collate_fn, num_workers=cfg.NUM_WORKERS,
                                       pin_memory=True)
    ...
    if cfg.DATASET.DATASET_FILE == "coco_panoptic":
        ...
        base_ds_cross = get_coco_api_from_dataset(dataset_val_cross)
    ...
    if cfg.RESUME and not cfg.FINETUNE: # [BUG] write after freezing cfgs
        ...
        if not cfg.EVAL:
            test_cross_stats, coco_evaluator = evaluate(
                model, criterion, postprocessors, data_loader_val_cross, base_ds_cross, device, cfg, prefix='init_eval_tgt_like_src'
            )
            test_tgt_stats, coco_evaluator = evaluate(
                model, criterion, postprocessors, data_loader_val_tgt, base_ds_tgt, device, cfg, prefix='init_eval_tgt'
            )
    ...
    # load checkpoint and start a new training
        ...
        if not cfg.EVAL:
            ...
            test_cross_stats, coco_evaluator = evaluate(model, criterion, postprocessors,
    ...
    if cfg.EVAL:
        ...
        test_cross_stats, coco_evaluator = evaluate(model, criterion, postprocessors,
                                                    data_loader_val_cross, base_ds_cross, device, cfg, prefix='eval_cross_epoch={CURRENT_EPOCH}')
        if cfg.OUTPUT_DIR:
    ...
    for epoch in range(START_EPOCH, cfg.TRAIN.EPOCHS):
        ...
        test_cross_stats, coco_evaluator = evaluate(
            model, criterion, postprocessors, data_loader_val_cross, base_ds_cross, device, cfg, prefix=f'eval_cross_epoch={epoch}'
        )
