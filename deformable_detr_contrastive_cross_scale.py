    ...
    def forward(self, samples: NestedTensor, targets, cur_iter_num, total_iter_num):
        ...
        if self.training:
            ...
            for src in srcs:
                ...
                memory_reshaped_tmp = memory_flat.reshape(B,c,w,h)
                memory_reshaped.append(memory_reshaped_tmp)

            assert len(memory_reshaped) == 4
            # import pdb; pdb.set_trace()
            ...
            # list_of_scores = [torch.randn((1), device='cuda')]
            # list_of_labels = [[1]]
            # rescaled_boxes = [torch.randn((1,4), device='cuda')]
            ...
            spatial_scales = [1/2.0, 1/8.0, 1/16.0, 1/32.0]
            list_of_rois = [] # len()==4
            for m, scale in zip(memory_reshaped, spatial_scales):
                list_tmp = []
                for batch_idx in range(B):
                    # input dim: (N, C, H, W)
                    rois = torchvision.ops.roi_align(m[batch_idx].unsqueeze(0), [rescaled_boxes[batch_idx]], output_size=(7, 7), spatial_scale=scale, aligned=True).mean(3).mean(2)
                    list_tmp.append(rois)
                list_of_rois.append(list_tmp)
            ...
            for roi_group in list_of_rois:
                src_prototypes, tgt_prototypes = weighted_aggregate(B, list_of_labels, roi_group, list_of_scores, self.num_classes, self.hidden_dim)
                list_of_src_prototype.append(src_prototypes)
                list_of_tgt_prototype.append(tgt_prototypes)


            list_of_src_prototype = torch.stack(list_of_src_prototype)
            list_of_tgt_prototype = torch.stack(list_of_tgt_prototype)


            # TODO for testing
            # src_prototypes = torch.randn((9, 256), device='cuda')
            # tgt_prototypes = torch.randn((9, 256), device='cuda')
