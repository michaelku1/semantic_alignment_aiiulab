...
def main(cfg):
    ...
    if cfg.RESUME and not cfg.FINETUNE: # [BUG] write after freezing cfgs
        ...
        if not cfg.EVAL:
            ...
            if dataset_val_cross is not None:
                print('=== Cross Domain ===')
                test_cross_stats, _ = evaluate(
                    model, criterion, postprocessors, data_loader_val_cross, base_ds_cross, device, cfg,
                    prefix='init_eval_cross'
                )
            print('=== Target Domain ===')
    ...
    if cfg.EVAL:
        ...
        if dataset_val_cross is not None:
            ...
            test_cross_stats, coco_evaluator = evaluate(model, criterion, postprocessors,
                                                        data_loader_val_cross, base_ds_cross, device, cfg,
                                                        prefix=f'eval_cross_epoch={CURRENT_EPOCH}')
