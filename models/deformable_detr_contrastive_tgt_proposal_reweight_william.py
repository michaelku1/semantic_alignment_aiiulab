...
class DeformableDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor, targets: List[Dict], *args, **kwargs):
        ...
        outputs_class = torch.stack(outputs_classes)  # (#decoder_layer, N, #query, #class)
        outputs_coord = torch.stack(outputs_coords)  # (#decoder_layer, N, #query, 4)
        ...
        if self.training and self.uda:
            ...
            scale = 1 / 32
            B, C, h, w = srcs[-1].shape
            start_idx = h * w
            feat_map = memory[:, -start_idx:, :].view(B, h, w, C)  # (N, H_3, W_3, C)

            src_class_rois = get_src_class_rois(
                src_feat_map=feat_map[:B//2].permute(0, 3, 1, 2),  # (N, C, H_3, W_3)
                ...
                # debug=True,
                # samples=samples
            )  # {label: (#cls_box_in_batch, C, roi_h, roi_w)}
            src_class_prototypes = agg_src_rois_to_prototypes(src_class_rois, device=feat_map.device)  # {label: (C,)}

            tgt_output_class = outputs_class[:, B//2:]  # (#decoder_layer, N//2, #query, #class), only take target img to visualize
            tgt_output_coord = outputs_coord[:, B//2:]  # (#decoder_layer, N//2, #query, 4), only take target img to visualize

            tgt_class_rois = get_tgt_class_rois(
                tgt_feat_map=feat_map[:B//2].permute(0, 3, 1, 2),  # (N, C, H_3, W_3)
                tgt_output_logit=tgt_output_class[-1],  # (N//2, #query, #class), use the prediction from the last layer
                tgt_output_coord=tgt_output_coord[-1],  # (N//2, #query, 4), use the prediction from the last layer
                prob_threshold=0.8,
                roi_output_shape=7,
                roi_scale=scale,
                debug=False,
            ...
            outputs_class = outputs_class[:, :B//2]  # (#decoder_layer, N//2, #query, #class), only take source img to compute loss
            outputs_coord = outputs_coord[:, :B//2]  # (#decoder_layer, N//2, #query, 4), only take source img to compute loss
        ...
        # use the prediction from the last layer to compute loss
        # see `SetCriterion.loss_labels_bce()` & `SetCriterion.loss_boxes()`
        out = {
            'pred_logits': outputs_class[-1],  # (N//2, #query, #class)
            'pred_boxes': outputs_coord[-1],  # (N//2, #query, 4)
            'tgt_pred_logits': tgt_output_class[-1],  # (N//2, #query, #class), output to visualize
            'tgt_pred_boxes': tgt_output_coord[-1]  # (N//2, #query, 4), output to visualize
        }
