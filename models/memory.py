...
#                     query_update[i] = torch.sum(query[idx].squeeze(1), dim=0)
    ...
    def forward(self, query, keys, train=True):
        ...
        query = F.normalize(query, dim=1) # fea
        ...
        if train:
            ...
            return updated_query, updated_memory, softmax_score_query, softmax_score_memory
    ...
    def update(self, query, keys,train):
        ...
        if train:
            ...
            updated_memory = F.normalize(query_update + keys, dim=1) # update memories
    ...
    def read(self, query, updated_memory):
        ...
        # scores normalised along query/memory dimensions
        # the scores are used to weight queries or memories
        softmax_score_query, softmax_score_memory = self.get_score(updated_memory, query)

        query_reshape = query.contiguous().view(batch_size*h*w, dims)
        
        # use the scores to weight memory
