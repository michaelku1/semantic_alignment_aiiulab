...
from .deformable_transformer_category import build_deforamble_transformer
    ...
    def forward(self, samples: NestedTensor, targets, cur_iter_num, total_iter_num):
        ...
        if self.training:
            ...
            for batch_idx in range(B//2):
                ...
                rois = torchvision.ops.roi_align(srcs[0][batch_idx].unsqueeze(0), [rescaled_boxes_enc[batch_idx]], output_size=(7, 7), spatial_scale=1/32.0, aligned=True).mean(3).mean(2)
                list_of_rois_enc.append(rois)
            
            ### weighted aggregate
            src_prototypes_enc, alpha_values = weighted_aggregate_source(B, list_of_labels_enc, list_of_rois_enc, list_of_scores_enc, self.num_classes, self.hidden_dim)
            src_prototypes_enc = src_prototypes_enc.unsqueeze(0)
            src_prototypes_enc = torch.repeat_interleave(src_prototypes_enc, B, dim=0) # TODO to test using source for init first
        ...
        if self.debug:
            hs, memory, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, da_output, _, prototype_tokens= self.transformer(srcs, masks, pos, src_prototypes_enc, query_embeds)

        # eval mode
        elif not self.training and not self.debug:
            hs, memory, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, da_output, _, _ = self.transformer(srcs, masks, pos, None, query_embeds)
        ...
        if self.training:
            ...
            # TODO try not to use ema memory 
            if self.ema:
                import pdb; pdb.set_trace()
                updated_prototype_tokens = self.memory(prototype_tokens)
                updated_src_prototypes_enc = updated_prototype_tokens[0]
                updated_tgt_prototypes_enc = updated_prototype_tokens[1]

            else:
                # import pdb; pdb.set_trace()
                updated_src_prototypes_enc = prototype_tokens[:, 0] # (6, 9, 256)
                updated_tgt_prototypes_enc = prototype_tokens[:, 1] # (6, 9, 256)
...
class SetCriterion(nn.Module):
    ...
    def contrastive_loss(self, source, target, alpha_values, margin=1):
        ...
        for batch_idx in range(len(source)):
            source_tmp = source[batch_idx]
            target_tmp = target[batch_idx]
            # TODO for initial source and target zero entries, 
            for cls_idx in range(self.num_classes):
                # i gives a fixed class
                tmp_src_feat_1 = source_tmp[cls_idx, :] # per class prototype
                tmp_tgt_feat_1 = target_tmp[cls_idx, :] # per class prototype
                ...
                for cls_idx_next in range(cls_idx+1, self.num_classes):
                    tmp_src_feat_2 = source_tmp[cls_idx_next, :]
                    tmp_tgt_feat_2 = target_tmp[cls_idx_next, :]
    ...
    def forward(self, outputs, targets, mode='train'):
        ...
        if 'prototypes_dec' in outputs:
            ...
            losses[f'intra_class_dec'] = intra_loss_dec
            losses[f'inter_class_dec'] = inter_loss_dec
