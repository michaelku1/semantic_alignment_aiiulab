...
from .deformable_transformer_ck import build_deforamble_transformer
...
class DeformableDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor, targets):
        ...
        if self.debug:
            # hs: output of trasnformer decoder
            # memory: output of transformer encoder
        ...
        for lvl in range(hs.shape[0]):
            ...
            outputs_class = self.class_embed[lvl](hs[lvl])  # final predictions, linear layer
        ...
        outputs_class = torch.stack(outputs_classes)  # torch.Size([6, 2, 300, 4])
        outputs_coord = torch.stack(outputs_coords)  # torch.Size([6, 2, 300, 4])
        ...
        if self.training:
            ...
            last_layer_out = outputs_class[-1]  # bs is after layer num  # it has all output of decoder layer, -1 for last layer
            outputs_class_conf = F.softmax(last_layer_out, -1)

            thresh = 0.6

            # size: [torch.Size([31, 2]), torch.Size([11, 2])]
            # 31: # of index in source
            # 11: # of index in target
            # 前半部是 source，後半部是 target
            # source & target 合併成一個 batch
            ...
            for batch_idx in range(B):
                ...
                boxes_rescaled = [box_ops.box_cxcywh_to_xyxy(boxes)]  # src only, batch size = 1 # normalized
                # and from relative [0, 1] to absolute [0, height] coordinates
                img_sizes = torch.stack([t["size"] for t in targets], dim=0)  # target: ground truth labels
                img_h, img_w = img_sizes.unbind(1)
                scale_fct = torch.stack([img_w, img_h, img_w, img_h], dim=1)

                for b in range(boxes_rescaled[0].shape[0]):
                    boxes_rescaled[0][b] *= scale_fct[0]  # batch_size = 1, one image  # unnormalized
            ...
            for batch_idx in range(B):
                ...
                # done for source and target feature
            ...
            assert len(weighted_rois_source) == len(source_labels) & len(weighted_rois_target) == len(target_labels), "label and roi lists are not one-to-one"
            
            # list of tensors: stores some rois for each class
            source_rois = []  # (bz, num_class, num_roi), source_rois[bz_idx][0] stores a list of RoI with label 0, source_rois[1] stores a list of RoI with label 1
            target_rois = []  # (bz, num_class, num_roi), target_rois[bz_idx][0] stores a list of RoI with pseudo-label 0, target_rois[1] stores a list of RoI with pseudo-label 1
            ...
            # batch length
            for i in range(len(source_rois)):  # i: bz_idx
                roi_sample_tmp = source_rois[i]  # some rois for a single class
                # some rois
                for j in range(len(roi_sample_tmp)):  # j: label_idx
                    ...
                    src_prototypes[i][source_labels[i][j]] = aggregate  # (d,)
