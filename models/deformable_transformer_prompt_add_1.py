...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, encoder_prompt_embeddings=None, decoder_prompt_embeddings=None):
        ...
        memory, space_query, channel_query = self.encoder(
            src_flatten, space_query, channel_query, spatial_shapes, level_start_index, valid_ratios, lvl_pos_embed_flatten, mask_flatten, encoder_prompt_embeddings
        )
        ...
        hs, inter_references, instance_query = self.decoder(
            tgt, instance_query, reference_points, memory, spatial_shapes, level_start_index, valid_ratios, query_embed, mask_flatten, decoder_prompt_embeddings
        )
...
class DeformableTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, space_query, channel_query, pos, reference_points, spatial_shapes, level_start_index, padding_mask=None, prompt_embed=None):
        ...
        if prompt_embed is not None:
            src = add_1_prompt_embed_to_src(src, level_start_index, prompt_embed)
...
class DeformableTransformerEncoder(nn.Module):
    ...
    def forward(self, src, space_query, channel_query, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None, prompt_embeddings=None):
        ...
        for layer_idx, layer in enumerate(self.layers):
            prompt_embed = None
            if prompt_embeddings is not None:
                if layer_idx < len(prompt_embeddings):
                    prompt_embed = prompt_embeddings[layer_idx]
            # prompt_embed:
            #   (num_feature_levels, d_model)
            #   None
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, instance_query, query_pos, reference_points, src, src_spatial_shapes, level_start_index, src_padding_mask=None, prompt_embed=None):
        ...
        if prompt_embed is not None:
            tgt = add_1_prompt_embed_to_tgt(tgt, prompt_embed)
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, instance_query, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            prompt_embed = None
            if prompt_embeddings is not None:
                if lid < len(prompt_embeddings):
                    prompt_embed = prompt_embeddings[lid]     
            # prompt_embed:
            #   (num_queries, hidden_dim)
            #   None

            output, instance_query = layer(
                output, instance_query, query_pos, reference_points_input, src, src_spatial_shapes, src_level_start_index, src_padding_mask, prompt_embed
            )
...
def build_deforamble_transformer(cfg):
    return DeformableTransformer(
        ...
        deep_shared=cfg.MODEL.VISUAL_PROMPT.DEEP_SHARED)
