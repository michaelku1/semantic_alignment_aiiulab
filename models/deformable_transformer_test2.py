...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        encoder_layer = DeformableTransformerEncoderLayer(d_model, dim_feedforward,
                                                          dropout, activation,
                                                          num_feature_levels, nhead, enc_n_points, space_align, channel_align) #return src, space_q, channel_q

        self.encoder = DeformableTransformerEncoder(encoder_layer, num_encoder_layers) #repeat encoder_layer 6 times

        decoder_layer = DeformableTransformerDecoderLayer(d_model, dim_feedforward,
                                                          dropout, activation,
                                                          num_feature_levels, nhead, dec_n_points, instance_align) #return tgt, instance_q
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None):
        ...
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):
            ...
            mask = mask.flatten(1) # let sequences length to be same
            #mask=[2,14028]
            #mask=[2,3528]
            #mask=[2,882]

            pos_embed = pos_embed.flatten(2).transpose(1, 2)
            #pos_embed=[2,14028,256]
            #pos_embed=[2,3528,256]
            #pos_embed=[2,882,256]

            lvl_pos_embed = pos_embed + self.level_embed[lvl].view(1, 1, -1) #level_embed = multi_scale level(concate multi scale feature) lvl means level
            #lvl_pos_embed=[2,14028,256]

            lvl_pos_embed_flatten.append(lvl_pos_embed)
            #lvl_pos_embed_flatten=[2,84*167,256]
            #lvl_pos_embed_flatten=[2,42*84,256]
            #lvl_pos_embed_flatten=[2,21*42,256]
            #lvl_pos_embed_flatten=[2,11*21,256]
            src_flatten.append(src)
            #src_flatten=[2,84*167,256]
            #src_flatten=[2,42*84,256]
            #src_flatten=[2,21*42,256]
            #src_flatten=[2,11*21,256]
            mask_flatten.append(mask)
            #mask_flatten=[2,84*167]
        ...
        if self.two_stage:
            ...
            query_embed, tgt = torch.split(query_embed, c, dim=1) #tgt = object_q  query_embed = positional embedding
