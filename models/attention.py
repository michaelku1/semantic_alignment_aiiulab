...
class SingleHeadSiameseAttention(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        ### computes the attention between src features category codes, and then use the 
        ### attention weights to weight the category codes
        output, _, _ = self.attention(q, k, v)


        ##s# use the attention weights to weight the task position encoding
        # tp, _, _ = self.attention(q, k, tsp)
        ...
        output2 = self.linear2(residual - output) # residual = q (the original input)
